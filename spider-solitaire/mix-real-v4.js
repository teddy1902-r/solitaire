...
function realBuildCandidate(level) {
  ...
  /*
   * On mélange chaque profondeur AVANT de retirer la prochaine couche de 10
   * cartes. C'est ce qui casse réellement les K-Q-J-10... visibles au départ.
   */
  ...
  /* Gros mélange supplémentaire des 54 cartes qui restent sur la table. */
  cols = realScrambleStage(
    cols,
  ...
  /*
   * Les retraits ont été enregistrés dans l'ordre deal5 -> deal1.
   * La pioche doit ressortir dans l'ordre deal1 -> deal5 avec stock.pop().
   */
...
/*
 * Simulateur de la solution connue. Il rejoue exactement à l'envers les
 * événements utilisés pour construire la partie, y compris les 5 distributions.
 */
guaranteedSimulateSolution = function(candidate) {
  ...
  for (let eventIndex = candidate.solutionEvents.length - 1;
    ...
    /* Inverse du déplacement utilisé pendant la construction. */
    const from = event.to;
    const to = event.from;
...
/*
 * On conserve la meilleure distribution testée. Même si le seuil de mélange
 * maximal n'est pas atteint sur un téléphone lent, on renvoie une partie valide
 * au lieu d'afficher un plateau vide.
 */
guaranteedBuildDeal = function(level) {
  ...
  for (let attempt = 0; attempt < 55; attempt++) {
    ...
    /*
     * 44 frontières maximum sur les 54 cartes. Ce seuil interdit les plateaux
     * constitués de longues colonnes K-Q-J-10-9-8 comme sur la capture.
     */
  ...
  /* Secours : meilleure partie gagnable trouvée, sans jamais vider le plateau. */
  best.hiddenCount = guaranteedAddHiddenCards(best, level.hiddenTarget);
  if (!guaranteedSimulateSolution(best)) {
    /* Si le masquage gêne la solution connue, toutes les cartes restent visibles. */
